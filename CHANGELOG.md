...
- **Child-table decomposition is no longer silently skipped when the sampled value is an empty list.** `_decompose_json_columns` decided whether to split a STRUCT/LIST column into its own child table by sampling a single arbitrary row (`LIMIT 1` with no `ORDER BY`) whose predicate excluded NULLs but not empty lists (`[]`). Shopify GraphQL returns `[]` for `transactions`, `paymentGatewayNames`, `tags`, etc. on many rows, so `not sample[0]` was true whenever the arbitrary sample happened to land on an empty-list row — and the **entire child table was dropped for the whole run**, logged only at `debug` level while the job reported success (classic silent data loss). Observed in production (project 1941 backfill, CZ 2022 order chunk): `order_transactions` lost ~56.6k rows and `order_payment_gateway_names` was absent entirely, on a green job. List-typed columns now sample a non-empty value (`AND len("col") > 0`), and a genuinely empty column now logs the skip at `info` level (L1-152).
  > **Behavior note.** Configs whose child tables were being intermittently dropped will now emit them consistently. On projects with prior partial runs this may surface previously-missing rows — a recovery, not a regression.
